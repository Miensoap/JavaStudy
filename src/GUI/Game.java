...
public class Game implements ActionListener{
	...
	void showAnswer() {
		...
		for(int i=0 ; i<getsu;i++) {
			for(int j=0;j<getsu;j++) {
				btn[i][j].setIcon(new ImageIcon ("src/GUI/b"+answer[i][j]+".PNG"));
			...
			for(int j=0;j<getsu;j++) {
				if(b == btn[i][j]) {
					...
					if(firstClick == null) { //첫번째 선택시
						...
						firstRow = i;
						firstCol = j;
						firstClick.setIcon(new ImageIcon ("src/GUI/b"+answer[firstRow][firstCol]+".PNG"));
						
					}else { //두번째 선택시
						if(answer[i][j]==answer[firstRow][firstCol]) {
							firstClick.setBackground(Color.gray);
							b.setIcon(new ImageIcon ("src/GUI/b"+answer[firstRow][firstCol]+".PNG"));
